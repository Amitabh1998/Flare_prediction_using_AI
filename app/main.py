...
# Custom CSS with Tailwind and Font Awesome
st.markdown("""
...
with tab1:
    ...
    with st.container():
        ...
        fig = px.bar(x=flare_counts.index, y=flare_counts.values, color=flare_counts.index,
                     color_discrete_map={0: '#3b82f6', 1: '#ef4444'},
                     labels={'x': 'Flare Status', 'y': 'Count'})
    ...
    with st.container():
        ...
        if "failed to predict flares" in report:
            st.markdown(
                '<div class="warning-box"><i class="fas fa-exclamation-triangle mr-2"></i>The model is predicting only "No Flare". Try retraining or adjusting inputs.</div>',
                unsafe_allow_html=True
            )
...
with tab2:
    ...
    with st.container():
        ...
        select_subject = st.selectbox("Select Subject", df['subject'].unique(), key="subject_select",
                                      help="Choose a subject to view their pain trend over time")
        subject_df = df[df['subject'] == select_subject]
        fig = px.line(subject_df, x='day', y='pain', title=f"Pain Trend for {select_subject}",
                      markers=True, color_discrete_sequence=['#1f2937'])
    ...
    with st.container():
        ...
        fig = px.bar(melt_df, x='Metric', y='Value', color='flare_next_day', barmode='group',
...
with tab3:
    ...
    with st.expander("Enter Your Data", expanded=True):
        ...
        with col1:
            ...
            total_sleep = st.slider(
                "Total Sleep (hours)", min_value=5.0, max_value=9.0, value=6.5, step=0.1,
                help="Total hours of sleep in a night",
                key="total_sleep",
                format="%.1f"
            )
            ...
            deep_sleep = st.slider(
                "Deep Sleep (hours)", min_value=0.5, max_value=2.0, value=1.0, step=0.1,
                help="Hours spent in deep sleep",
                key="deep_sleep",
                format="%.1f"
            )
            st.markdown(f"<p class='text-sm text-gray-500'>Selected: {deep_sleep:.1f} hours</p>", unsafe_allow_html=True)
            
            rem_sleep = st.slider(
                "REM Sleep (hours)", min_value=0.5, max_value=2.0, value=1.0, step=0.1,
                help="Hours spent in REM sleep",
                key="rem_sleep",
                format="%.1f"
            )
            st.markdown(f"<p class='text-sm text-gray-500'>Selected: {rem_sleep:.1f} hours</p>", unsafe_allow_html=True)
            
            wake = st.slider(
                "Wake Time (hours)", min_value=15.0, max_value=19.0, value=17.0, step=0.1,
                help="Hours spent awake during sleep period",
                key="wake",
                format="%.1f"
            )
            ...
            sleep_hours = st.slider(
                "Sleep Hours (normalized)", min_value=0.0, max_value=1.2, value=0.5, step=0.01,
                help="Normalized sleep duration metric",
                key="sleep_hours",
                format="%.2f"
            )
        ...
        with col2:
            ...
            pain = st.slider(
                "Pain Level (0-10)", min_value=0, max_value=10, value=5,
                help="Self-reported pain level (0 = none, 10 = severe)",
                key="pain"
            )
            st.markdown(f"<p class='text-sm text-gray-500'>Selected: {pain}</p>", unsafe_allow_html=True)
            
            fatigue = st.slider(
                "Fatigue Level (0-10)", min_value=0, max_value=10, value=5,
                help="Self-reported fatigue level (0 = none, 10 = extreme)",
                key="fatigue"
            )
            st.markdown(f"<p class='text-sm text-gray-500'>Selected: {fatigue}</p>", unsafe_allow_html=True)
            
            mood = st.slider(
                "Mood Level (0-10)", min_value=0, max_value=10, value=5,
                help="Self-reported mood (0 = poor, 10 = excellent)",
                key="mood"
            )
            st.markdown(f"<p class='text-sm text-gray-500'>Selected: {mood}</p>", unsafe_allow_html=True)
            
            treatment = st.radio(
                "Treatment Status", [0, 1], format_func=lambda x: "Yes" if x == 1 else "No",
                help="Are you receiving treatment? (1 = Yes, 0 = No)",
                key="treatment"
            )
        ...
        if total_sleep < deep_sleep + rem_sleep:
            st.markdown(
                '<div class="warning-box"><i class="fas fa-exclamation-triangle mr-2"></i>Total sleep must be greater than or equal to deep sleep + REM sleep.</div>',
                unsafe_allow_html=True
            )
                ...
                with st.spinner("Predicting..."):
                    ...
                    st.markdown(
                        ...
                        <div class="prediction-box">
                            ...
                            <p class="text-gray-500 text-sm">Raw Output: {output if output is not None else 'N/A'}</p>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
                    if prediction == "No Flare Expected":
                        st.markdown(
                            '<div class="warning-box"><i class="fas fa-exclamation-triangle mr-2"></i>Model predicted "No Flare". If this persists for all inputs, the model may need retraining.</div>',
                            unsafe_allow_html=True
                        )